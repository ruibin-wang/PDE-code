...
function ptMatrix = CounterLineDeformation(countLineAttachBd, aixsT, deltT, bdDirDefNum, deformAngle, coff)
...
%%% 原始的面片延拓点
originalExtendA4_F4  =    [Mesh_1(meshIndex_1(1,:), :); Mesh_4(meshIndex_4(:,end), :); Mesh_7(meshIndex_7(:,end), :); ...
                              Mesh_10(meshIndex_10(:,end), :); Mesh_13(meshIndex_13(:,end), :)];


originalExtendA1_O1_A3  =    [Mesh_16(meshIndex_16(:,1), :); Mesh_17(meshIndex_17(:,1), :)];

%%% 简化之后的延拓点
extendA1_F1          =    findNearPoint(A1_F1, originalExtendA1_F1, 100000);        %%  边界边的延拓点
extendA4_F4          =    findNearPoint(A4_F4, originalExtendA4_F4, 100000);        %%  边界边的延拓点
extendA1_O1_A3       =    findNearPoint(A1_O1_A3, originalExtendA1_O1_A3, 100000);        %%  边界边的延拓点
...
extendF1_F4          =    findNearPoint(F1_F4, originalExtendF1_F4, 100000);        %%  边界边的延拓点
boundAttachToF1_F4{2,2} = extendF1_F4;

A3_A4 = [avePoint_1(1,:); avePoint_1(end:-1:31,:)];

originalExtendA3_A4  =    Mesh_1(meshIndex_1(:,1), :);
extendA3_A4          =    findNearPoint(A3_A4, originalExtendA3_A4, 100000);        %%  边界边的延拓点
...
%%% testcode 为了减少计算时间，将eval的部分单独拿出来进行运算，计算时间大幅减少
...
testTemp = cell(length(countLineAttachBd), 1);



for indexI = 1:length(countLineAttachBd)
%     tempBoundAttachToCountLine{indexI,1} = eval(vpa(countLineAttachBd{1,indexI}, 4));  %% 避免误差
    tempBoundAttachToCountLine{indexI,1} = round(countLineAttachBd{1,indexI}, 2);  %% 避免误差
end

for indexI = 1:length(avePointSet)
%     tempAvePointSet{indexI,1} = eval(vpa(avePointSet{indexI,2}, 4));
...
% eval(['ptMatrix_', countLineAttachBd{2,1}, '_', aixsT, ' = ptMatrix;']);
% eval(['save ', 'ptMatrix_', countLineAttachBd{2,1}, '_', aixsT, '.txt',...
%               '  ptMatrix_', countLineAttachBd{2,1}, '_', aixsT, '  -ascii']);

% hold on; scatter3(ptMatrix(:,1), ptMatrix(:,2), ptMatrix(:,3), 'filled')
% hold on; scatter3(deformExtendCountLineSet(:,1), deformExtendCountLineSet(:,2), deformExtendCountLineSet(:,3), 'filled')
